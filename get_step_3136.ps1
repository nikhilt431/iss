...
foreach ($line in $lines) {
    try {
        ...
        if ($null -ne $obj -and $obj.step_index -eq 3136) {
            $obj.content | Out-File -FilePath 'C:\Users\Nikhi\.gemini\antigravity\scratch\bible-quiz-app\step_3136_content.txt' -Encoding utf8
            Write-Output "Extracted step 3136!"
        }
    } catch {}
}
